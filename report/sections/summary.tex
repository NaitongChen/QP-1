...
Now suppose $\hat{\mu}_\lambda$ depends on $\lambda\in\Lambda$, which controls the degree of regularization to our estimator, and assume $\sigma$ is known, we can find the optimal $\lambda$, denoted $\hat{\lambda}$ by
\[
